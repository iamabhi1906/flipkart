...
export default function PrimarySearchAppBar() {
    ...
    return (
        <Box sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
            <AppBar position="fixed" sx={{ backgroundColor: '#2874F0', height: '56px' }}>
                <Toolbar sx={{ maxWidth: '1248px', margin: '0 auto', width: '100%', height: '56px' }}>
                    <Typography
                        variant="h6"
                        noWrap
                        component="div"
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center' }}>
                        {user ? (
                            ...
                            <Box sx={{ display: 'flex', gap: 1 }}>
                                <Button color="inherit" onClick={() => router.push('/login')}>Login / Sign Up</Button>
                            </Box>
                        )}
                    </Box>
                </Toolbar>
